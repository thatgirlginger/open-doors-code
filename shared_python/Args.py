...
class Args(object):
    ...
    def _process_args(self):
        ...
        for name, helptext in argdict.items():
            parser.add_argument(
                "-d" + name.split("_")[1][0], "--" + name, type=str, help=helptext
            )
        ...
        parser.add_argument(
            ...
            "--properties_file",
            type=str,
            help="Load properties from specified file (ignores all other arguments)",
        )

        # General archive-specific settings
        parser.add_argument(
            "-a", "--archive_type", type=str, choices=["AA"], help="Type of archive: AA"
        )
        parser.add_argument(
            "-df", "--default_fandom", type=str, help="Default fandom to use"
        )
        parser.add_argument(
            "-n",
            "--archive_name",
            type=str,
            help="Name of the original archive (used in the temporary site)",
        )
        ...
        parser.add_argument(
            ...
            "--db_input_file",
            type=str,
            help="Path to input file (ARCHIVE_DB.pl for AA)",
        )
        parser.add_argument(
            "-o", "--output_folder", type=str, help="Path for output files"
        )
        parser.add_argument(
            "-od",
            "--output_database",
            type=str,
            help='Name of the database the final tables should be created in (default "od_sgf")',
        )
        ...
        parser.add_argument(
            ...
            "--tag_fields",
            type=str,
            help="List of tag field(s) in original db (comma-delimited)",
        )
        parser.add_argument(
            "-fc",
            "--character_fields",
            type=str,
            help="List of character field(s) in original db (comma-delimited)",
        )
        parser.add_argument(
            "-fr",
            "--relationship_fields",
            type=str,
            help="List of relationship field(s) in original db (comma-delimited)",
        )
        parser.add_argument(
            "-ff",
            "--fandom_fields",
            type=str,
            help="List of fandom field(s) in original db (comma-delimited)",
        )
        parser.add_argument(
            "-wf",
            "--fields_with_fandom",
            type=str,
            help="List of output tag fields where the fandom should be listed too (comma-delimited)",
        )

        # Wrangling and search processing
        parser.add_argument(
            "-t", "--tag_input_file", type=str, help="Path to tag renaming input CSV"
        )
        parser.add_argument(
            "-si",
            "--story_ids_to_remove",
            type=str,
            help="Location of the text file containing the story ids to remove",
        )
        parser.add_argument(
            "-bi",
            "--bookmark_ids_to_remove",
            type=str,
            help="Location of the text file containing the bookmark ids to remove",
        )
        ...
        parser.add_argument(
            ...
            "--chapters_path",
            type=str,
            help="Location of the text files containing the stories",
        )
        parser.add_argument(
            "-cf",
            "--chapters_file_extensions",
            type=str,
            help='File extension(s) of the text files containing the stories (eg: "txt, html")',
        )
        ...
        args.archive_name = (
            input("Name of the original archive (used in export file names): ")
    ...
    def args_for_01(self):
        while self.args.db_input_file is None or not os.path.isfile(
            self.args.db_input_file
        ):
            self.args.db_input_file = input(
                "Path to the input file (ARCHIVE_DB.pl for AA): "
            )
    ...
    def args_for_03(self):
        ...
        while self.args.output_folder is None or not os.path.isdir(
    ...
    def args_for_04(self):
        while self.args.tag_input_file is None or not os.path.isfile(
            self.args.tag_input_file
        ):
    ...
    def args_for_05(self):
        ...
        if self.args.story_ids_to_remove is None:
            self.args.story_ids_to_remove = input(
                "Location of the text file containing the story ids to remove:"
            )
        self._print_args(self.args)
        return self.args
    ...
    def args_for_06(self):
        ...
        if self.args.default_fandom is None:
            ...
            self.args.default_fandom = (
                "" if self.args.default_fandom is None else self.args.default_fandom
            )
        self._print_args(self.args)
        return self.args
    ...
    def args_for_07(self):
        ...
        if (
            self.args.chapters_path is not None
            and self.args.chapters_file_extensions is None
        ):
            self.args.chapters_file_extensions = input(
                'File extension(s) of the text files containing the stories (eg: "txt, html"):'
            )
        self._print_args(self.args)
        return self.args
